...
implement_uniform_block!(SimRBCell, matID, orig_off, pos, prev_pos, rb_idx);
...
pub struct Simulation {
    ...
    pub collision_data: texture::IntegralTexture2d,
...
impl Simulation {
    pub fn new(display: &RendererDisplay, size: (u32, u32)) -> Self {
        ...
        let collision_tex_scale = 1;
        let colsize = (size.0 / collision_tex_scale, size.1 / collision_tex_scale);
        let coldata : Vec<i32> = vec![0; (colsize.0 * colsize.1 * 3) as usize];
        let collision_data = texture::IntegralTexture2d::with_format(display, texture::RawImage2d::from_raw_rgb(coldata, colsize), texture::UncompressedIntFormat::I32, no_mip).unwrap();
        ...
        let rb_size = 15;
        for rb_y in -rb_size..=rb_size {
            for rb_x in -rb_size..=rb_size {
    ...
    pub fn run(&mut self) {
        ...
        let collision_img = self.collision_data.image_unit(glium::uniforms::ImageUnitFormat::R32I).unwrap().set_access(read_write);
